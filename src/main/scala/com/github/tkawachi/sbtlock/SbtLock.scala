...
object SbtLock {
  ...
  def doLock(allModules: Seq[ModuleID], depsHash: String, lockFile: File, log: Logger): File = {
  ...
  def lockFile(state: State): File = {
    ...
    val lockFileName = EvaluateTask.getSetting(SbtLockKeys.sbtLockLockFile, DEFAULT_LOCK_FILE_NAME, extracted, buildStruct)
    new File(buildUnit.localBase, lockFileName)
  }

  val checkDepUpdates = (state: State) => {
    // TODO run for all projects
    val (nextState, _) = Project.extract(state).runTask(SbtLockKeys.checkLockUpdate, state)
    nextState
  }
}
